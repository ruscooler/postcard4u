...
function igthemes_posts_navigation() {
    ...
    <?php }// end pagination
        ...
        the_posts_navigation(array(
            'prev_text'          => __( 'Previous','store-wp' ),
            'next_text'          => __( 'Next','store-wp' ),
        ));
    }
    echo "</div>";
}
...
function igthemes_posted_on() {
    //AUTHOR
    $byline = sprintf(
          esc_html_x( apply_filters('igthemes-author','igthemes-author-text') . '%s', 'post author', 'store-wp' ),
	...
	$posted_on = sprintf(
		esc_html_x( apply_filters('igthemes_date', 'igthemes_date_text') . '%s', 'post date', 'store-wp' ),
...
function igthemes_entry_footer() {
	...
	if ( 'post' === get_post_type() && apply_filters( 'igthemes_post_cat', true ) ) {
			printf( '<span class="cat-links">' . esc_html__( apply_filters('igthemes_cat_links', 'igthemes-cat-text') . '%1$s', 'store-wp' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'store-wp' ) );
		if ( $tags_list && apply_filters( 'igthemes_post_tags', true ) ) {
			printf( '<span class="tags-links">' . esc_html__( apply_filters('igthemes_tags_links', 'igthemes-tags-text') . '%1$s', 'store-wp' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
    //commnet
    if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) && apply_filters( 'igthemes_post_comments_link', true ) ) {
		...
		comments_popup_link( sprintf( wp_kses( __( 'Leave a comment<span class="screen-reader-text"> on %s</span>', 'store-wp' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
		echo '</span>';
	}
    //edit link
    edit_post_link(
