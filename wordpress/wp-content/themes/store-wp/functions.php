...
function store_wp_setup() {
   /*
     ...
     * to change 'store-wp' to the name of your theme in all the template files.
     */
    load_theme_textdomain( 'store-wp' );
    ...
    register_nav_menus( array(
        'primary' => esc_html__( 'Primary', 'store-wp' ),
...
function store_wp_content_width() {
    $GLOBALS['content_width'] = apply_filters( 'store_wp_content_width', 1140 );
}
...
function store_wp_widgets_init() {
    register_sidebar( array(
        'name'          => esc_html__( 'Sidebar', 'store-wp' ),
        'id'            => 'sidebar-1',
        'description'   => esc_html__( 'Add widgets here.', 'store-wp' ),
    ...
    register_sidebar( array(
        'name'          => esc_html__( 'Header widget area', 'store-wp' ),
        'id'            => 'header-widget',
        'description'   => esc_html__( 'Add widgets here.', 'store-wp' ),
        'before_widget' => '<section id="%1$s" class="widget %2$s">',
        'after_widget'  => '</section>',
    ...
    for ( $i = 1; $i <= intval( 4 ); $i++ ) {
        register_sidebar( array(
            'name' 				=> sprintf( __( 'Footer %d', 'store-wp' ), $i ),
            'id' 				=> sprintf( 'footer-%d', $i ),
            'description' 		=> sprintf( esc_html__( 'Widgetized Footer Region %d.','store-wp' ), $i ),
            'before_widget'     => '<section id="%1$s" class="widget %2$s">',
            'after_widget' 		=> '</section>',
            'before_title' 		=> '<h3 class="widget-title">',
            'after_title' 		=> '</h3>',
    ...
    if( is_edd_activated() || is_woocommerce_activated() ) {
        register_sidebar( array(
            'name'          => esc_html__( 'Shop widget area', 'store-wp' ),
            'id'            => 'sidebar-shop',
            'description'   => esc_html__( 'Add widgets here.', 'store-wp' ),
            'before_widget' => '<section id="%1$s" class="widget %2$s">',
            'after_widget'  => '</section>',
...
function store_wp_scripts() {
    wp_enqueue_style( 'store-wp-style', get_stylesheet_uri() );

    wp_enqueue_script( 'store-wp-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

    wp_enqueue_script( 'store-wp-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );

    if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
        wp_enqueue_script( 'comment-reply' );
    }
    wp_enqueue_script( 'store-wp-theme', get_template_directory_uri() . '/js/theme.js', array('jquery'), '1.0', true );
...
add_action( 'wp_enqueue_scripts', 'store_wp_scripts' );

//Gooogle fonts
function store_wp_google_fonts() {
        wp_enqueue_style( 'store-wp-fonts', '//fonts.googleapis.com/css?family='. apply_filters( 'igthemes_google_font', 'Open+Sans:300,300i,400,400i,700,700i&subset=latin-ext'));
}

add_action( 'wp_enqueue_scripts', 'store_wp_google_fonts' );
