...
function avrora_wp_title($title, $sep)
{
    ...
    if ($paged >= 2 || $page >= 2)
        $title = "$title $sep " . sprintf(__('Страница %s', 'avrora'), max($paged, $page));
...
if (!function_exists('avrora_credits')) :
    ...
    {
        $text = 'При поддержке <a href="' . esc_url('https://wphost.me/mancgi/partnerprogram?partner=24&amp;project=1') . '">Wordpress хостинг</a>. Все права защищены.';
    ...
    function avrora_content_nav($html_id)
    {
        ...
        if ($wp_query->max_num_pages > 1) : ?>
            <nav id="<?php echo $html_id; ?>" class="navigation" role="navigation">
                <div
                    class="nav-previous"><?php next_posts_link(__('<span class="meta-nav">&larr;</span> Предыдущие записи', 'avrora')); ?></div>
                <div
                    class="nav-next"><?php previous_posts_link(__('Следующие записи <span class="meta-nav">&rarr;</span>', 'avrora')); ?></div>
